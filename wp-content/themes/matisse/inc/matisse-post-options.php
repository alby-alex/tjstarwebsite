...
function post_options_api_example() {
	...
	$post_options->register_post_options_section( 'post-f', __( 'Post Format options', 'matisse' ),1 );
	$post_options->add_section_to_post_type( 'post-f', 'post' );
    
    $post_options->register_post_options_section( 'page-o', __( 'Matisse page options', 'matisse' ),1 );    
...
/*    Page slideshow  */
	...
	$post_options->register_post_option( array( 
		'id' => 'slide-page',
		'title' => __( 'Featured', 'matisse' ),
		'section' => 'page-o',
		'callback' => $post_fields->checkbox( array(
			'label' => __( 'This is a featured post', 'matisse' ),
			'description' => __( 'Check this to feature the post in the highlights section on the homepage.', 'matisse' )
		) )
	) );
	...
	$post_options->register_post_option( array( 
		'id' => 'pf-link',
		'title' => __( 'Post format - Link', 'matisse' ),
		'section' => 'post-f',
		'callback' => $post_fields->text( array(
			'description' => __( 'Paste link', 'matisse' )
		) )
	) );
/*    Post Format - Quote Author  */
	$post_options->register_post_option( array( 
		'id' => 'pf-quote-title',
		'title' => __( 'Post format - Quote - Author', 'matisse' ),
		'section' => 'post-f',
		'callback' => $post_fields->text( array(
			'description' => __( 'If you choose the format of the post - quotation - and you want to add the author you must write his name.', 'matisse' )
		) )
	) );
/*    Post Format - Quote link  */		
	$post_options->register_post_option( array( 
		'id' => 'pf-quote',
		'title' => __( 'Post format - Quote - Link', 'matisse' ),
		'section' => 'post-f',
		'callback' => $post_fields->text( array(
			'description' => __( 'Past the link into the source of quotation. The author of the quotation will be the link to the source, to make the link available fill in \"the Author quotation\" field.', 'matisse' )
		) )
	) );
...
/*    Post Format - Quote  */
/*----------*/
	if(!function_exists('matisse_post_quote')) {
function matisse_post_quote() {
global $post;
$post_options = get_post_options_api( '1.0.1' );
...
echo '<h2><cite><a href="'.$quote.'" title="'.esc_attr(sprintf(__('%s', 'matisse'), the_title_attribute('echo=0'))).'" rel="bookmark">'.$quoteauthor.'</a></cite></h2>';
}
	}
?>