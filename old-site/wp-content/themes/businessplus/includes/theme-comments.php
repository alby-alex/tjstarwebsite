...
<li class="comment wrap" id="comment-<?php comment_ID() ?>">
    ...
    <div class="col-right">
        <p class="comment-meta"><strong><?php comment_author_link() ?></strong><br/><small><?php echo get_comment_date(get_option( 'date_format' )) ?> <?php _e('at', 'themejunkie'); ?> <?php echo get_comment_time(get_option( 'time_format' )); ?> <a href="<?php echo get_comment_link(); ?>" title="<?php _e('Direct link to this comment', 'themejunkie'); ?>">#</a></small></p>    
		<?php comment_text() ?>
        <?php if ($comment->comment_approved == '0') : ?>
        <p><em><?php _e('Your comment is awaiting moderation.', 'themejunkie'); ?></em></p>
        <?php endif; ?>
